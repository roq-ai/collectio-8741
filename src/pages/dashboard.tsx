...
import { getFinances } from 'apiSdk/finances';
import { FinanceInterface } from 'interfaces/finance';

import { FinanceListPage as ShowCaseList } from 'pages/finances';
...
function DashboardPage() {
  ...
  const paymentStat = useSWR<PaginatedInterface<PaymentInterface>>(
    () => `/payments`,
    () => getPayments({ limit: 1, offset: 0 }),
  );

  const financeStat = useSWR<PaginatedInterface<FinanceInterface>>(
    () => `/finances`,
    () => getFinances({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Payments',
      result: paymentStat?.data?.totalCount,
      isLoading: paymentStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Finances',
      result: financeStat?.data?.totalCount,
      isLoading: financeStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`finances`} passHref legacyBehavior>
                  <Button
