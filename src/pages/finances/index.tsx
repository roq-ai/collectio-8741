...
type ColumnType = ColumnDef<FinanceInterface, unknown>;
...
export function FinanceListPage(props: FinanceListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<FinanceInterface>>(
    () => `/finances?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: FinanceInterface) => {
    if (hasAccess('finance', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/finances/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'budget', header: 'budget', accessorKey: 'budget' },
    { id: 'expense', header: 'expense', accessorKey: 'expense' },
    { id: 'income', header: 'income', accessorKey: 'income' },
    { id: 'profit', header: 'profit', accessorKey: 'profit' },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/finances/view/${record.id}`} passHref legacyBehavior>
                <Button
